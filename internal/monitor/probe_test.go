...
func TestProbeMonitorUsesInternalMaxConcurrency(t *testing.T) {
	...
	if atomic.LoadInt32(&factory.max) > int32(defaultWorkersPerCR) {
...
func TestProbeMonitorWorkersPerCROverride(t *testing.T) {
	scheme := runtime.NewScheme()
	if err := corev1.AddToScheme(scheme); err != nil {
		t.Fatal(err)
	}
	...
	result, err := (ProbeMonitor{Client: c, DBFactory: factory, WorkersPerCR: 2}).Check(context.Background(), resource, instances)
	if err != nil {
		t.Fatal(err)
	}
